...
import com.gmworks.gurug.fragment.CheckoutFragment;
                ...
                if (hash_from_response.equals(hash)) {
                    if (status.equals(Constant.SUCCESS)) {
                        if (from.equals(Constant.PAYMENT)) {
                            new PaymentActivity().PlaceOrder(activity, "PayUMoney", txnId, true, PaymentModelClass.sendParams, status);
                        ...
                        new PaymentActivity().PlaceOrder(activity, "PayUMoney", txnId, false, PaymentModelClass.sendParams, status);
